...
// build/dev/javascript/lustre_app/lustre_app.mjs
...
var UserIncrementCount = class extends CustomType {
};
var UserDecrementCount = class extends CustomType {
};
...
function update(model, msg) {
  if (msg instanceof UserIncrementCount) {
    return [model.withFields({ count: model.count + 1 }), get_cat()];
  } else if (msg instanceof UserDecrementCount) {
    return [
...
function view(model) {
  let number = to_string(model.count);
  let count = () => {
    return number + (() => {
      if (number === "1") {
        return " Cat";
  ...
  return div(
    toList([class$("w-full mx-auto flex flex-col gap-12")]),
    toList([
      h1(
        toList([class$("text-center text-2xl mb-4 font-bold")]),
        toList([text("Funny Cats")])
      ),
      div(
        toList([
          class$(
            "w-full mx-auto flex justify-center items-center gap-8"
          )
        ]),
        toList([
          button(
            toList([
              ...
              title("Get cats"),
              on_click(new UserIncrementCount())
            ]),
            toList([text("+")])
          ),
          text(count()),
          button(
            toList([
              class$("btn btn-outline btn-primary btn-sm"),
              title("Delete cats"),
              on_click(new UserDecrementCount())
            ]),
            toList([text("-")])
          )
        ])
        ...
        map(
          ...
          (cat) => {
            return [
              cat.url,
              a(
                toList([href(cat.url), target("_blank")]),
                toList([
                  img(
                    toList([
                      class$(
